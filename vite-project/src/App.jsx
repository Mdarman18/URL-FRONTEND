...
function App() {
  ...
  const HandleRedirect = async () => {
    if (url && url.id) {
      window.open(`http://localhost:8000/${url.id}`);
    }
  };
  ...
  const CopyLink = () => {
    if (url && url.id) {
      const shortUrl = `http://localhost:8000/${url.id}`;
    ...
    <>
      <div className="min-h-screen bg-purple-200 flex flex-col items-center ">
        <h1 className="text-2xl md:text-[2.34rem] font-semibold flex gap-[5px] mt-[20px]">
          URL SHORTNER{" "}
          <FaLink className="mt-[10px] cursor-pointer text-blue-900" />{" "}
        </h1>
        <div className="h-[200px] gap-[13px] flex flex-col w-full md:w-[800px] mt-[12px] bg-slate-200 rounded-[12px] p-4">
        ...
        <div className="fixed bottom-0 w-full md:w-auto flex justify-center md:justify-end p-4 md:right-5">
          <h1 className="flex items-center gap-2 md:gap-1 text-sm md:text-base">
            ...
            >
              <FaInstagram className="mt-[5px] ml-[5px]  cursor-pointer hover: text-red-800" /> @Only_Arman18
            </a>
          </h1>
